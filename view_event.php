<?php
// view_event.php - EVENT VIEWER (SAME DESIGN AS VIEW_UPDATE)
...
try {
    // Kunin ang full content ng event
    $stmt = $pdo->prepare("SELECT * FROM upcoming_events WHERE id = ?");
    $stmt->execute([$id]);
    $event = $stmt->fetch();
...
<head>
    ...
    <title><?php echo htmlspecialchars($event['title']); ?> - MSU Buug</title>
        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-10 mx-auto text-center">
                    ...
                    <h1 class="display-4 fw-bold mb-3"><?php echo htmlspecialchars($event['title']); ?></h1>
                    <div class="meta-info">
                        <p class="mb-2"><i class="fas fa-calendar me-2"></i><strong>Event Date:</strong> <?php echo date('F j, Y', strtotime($event['event_date'])); ?></p>
                        <p class="mb-2"><i class="fas fa-map-marker-alt me-2"></i><strong>Location:</strong> <?php echo htmlspecialchars($event['location']); ?></p>
                        <p class="mb-0"><i class="fas fa-clock me-2"></i><strong>Posted on:</strong> <?php echo date('F j, Y', strtotime($event['created_at'] ?? date('Y-m-d'))); ?></p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="row justify-content-center">
            <div class="col-lg-10">
                ...
                <div class="article-content">
                    <h3 class="text-center mb-4" style="color: var(--msu-green);">
                        <i class="fas fa-file-alt me-2"></i>Event Description
                    </h3>
                    <div class="content">
                        <?php 
                        // I-display ang full content with proper formatting
                        $content = htmlspecialchars($event['description']);
                    ...
                    <div class="mt-5 pt-4 border-top text-center">
                        ...
                        <a href="events.php" class="back-btn me-3" style="background: #28a745;">
